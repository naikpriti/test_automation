...
def fetch_tag_sha(tags_url, tag_name):
...
def create_branch(branch_url, branch_name, tag_name, tags_url):
    sha = fetch_tag_sha(tags_url, tag_name)
    if not sha:
        return
...
def update_files(owner, repo, branch_name, new_tag_name, ip_addresses, file_paths, variable_name):
    ...
    for file_path in file_paths:
        update_variables_tf(file_path, ip_addresses, variable_name)
    ...
    subprocess.run(["git", "add"] + file_paths + [version_txt_path])
    subprocess.run(["git", "commit", "-m", f"Update files and version.txt for release {new_tag_name}"])
...
def update_main_branch(owner, repo, ip_addresses, new_tag_name, file_paths, variable_name):
...
def create_release(create_release_url, branch_name, new_tag_name):
    data = {
...
def delete_branch(branch_url, branch_name):
...
def update_variables_tf(file_path, ip_addresses, variable_name):
    ...
    with open(file_path, 'w') as file:
        ...
        for line in lines:
            if f'variable "{variable_name}"' in line:
...
def fetch_and_process_json(region):
    ...
    # Step 10: Extract IPv4 addresses for the specified region
    ipv4_addresses = []
...
def main():
    ...
    for repo_config in config["repositories"]:
        ...
        variable = repo_config["variable"]
        region = repo_config["region"]
        ...
        new_tag_name, ip_addresses = fetch_and_process_json(region)

        # Update the main branch with the new IP addresses and version
        update_main_branch(owner, repo, ip_addresses, new_tag_name, files, variable)
        ...
        for (major_minor, (patch, tag_name)) in sorted_latest_versions:
            ...
            create_branch(branch_url, new_branch_name, tag_name, tags_url)
            
            # Update files in the new branch with the JSON file version
            update_files(owner, repo, new_branch_name, new_tag_name, ip_addresses, files, variable)
            
            # Create a new release from the new branch
            create_release(create_release_url, new_branch_name, new_release_tag)
            
            # Delete the branch after creating the release
            delete_branch(branch_url, new_branch_name)
